...
Generate Table 12: same as table1 but using ROSS with base postprocessor EBO instead of fDBD.
"""
import os
import shutil
import subprocess
...
try:
    ...
    out_path = os.path.join(tables_dir, 'table12.csv')
    table.to_csv(out_path, index=True)
    print(table)
    print(f"Saved Table 12 CSV to {out_path}")
finally:

...
Generate Table 6: analyze ROSS-fDBD post-processor on CIFAR-10 with different lambda values.
Runs eval_ood for each lambda, extracts only the 'ross' confidence metric, and writes table6.csv.
"""
import os
import shutil
import subprocess
...
# Save to CSV
out_path = os.path.join(tables_dir, 'table6.csv')
table.to_csv(out_path, index_label='lambda')
print(table)
print(f"Saved Table 6 CSV to {out_path}") 
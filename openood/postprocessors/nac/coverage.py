...
class KMNC(Coverage):
    ...
    def save(self, path, prefix="imagenet", aka_ln=None, verbose=True):
        ...
        # ensure hyper_dict is a plain nested dict (no Config objects) for safe pickling
        plain_hyper = {ln: dict(ln_hypers) for ln, ln_hypers in self.hyper_dict.items()}
        state = {
            'layer_size_dict': self.layer_size_dict,
            'hyper_dict': plain_hyper,

...
from ..criteria import Misclassification, TargetedMisclassification, Criterion
    ...
    def run(
        ...
        **kwargs: Any,
        ...
        if isinstance(criterion_, Misclassification):
            ...
            loss_fn = self.get_loss_fn(model, criterion_.labels)
        elif hasattr(criterion_, "target_classes"):
            gradient_step_sign = -1.0
            loss_fn = self.get_loss_fn(model, criterion_.target_classes)
        elif isinstance(criterion_, Criterion):
            # custom OOD criterion: minimize its output
            gradient_step_sign = -1.0
            def loss_fn(inputs):
                outputs = model(inputs)
                loss = criterion_(inputs, outputs)
                # cast boolean to float if needed
                try:
                    loss = loss.astype(ep.float32)
                except Exception:
                    pass
                return loss.sum()
        else:

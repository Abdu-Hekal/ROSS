...
# Criterion to minimize OOD score (gradient descent)
class OODMinConfidence(Criterion):
    def __init__(self):
        super().__init__()
    def __repr__(self) -> str:
        return "OODMaxConfidence()"
    def __call__(self, perturbed, outputs):
        # descent confidence
        return outputs[:, 0]
    
# Criterion to maximize OOD score (gradient ascent)
class OODMaxConfidence(Criterion):
    def __init__(self):
        super().__init__()
    def __repr__(self) -> str:
        return "OODMaxConfidence()"
    def __call__(self, perturbed, outputs):
        # negate to ascend confidence
        return -outputs[:, 0]